...
echo "Setup Jenkins..."
oc process jenkins-persistent -n openshift --param ENABLE_OAUTH=true --param MEMORY_LIMIT=2Gi --param VOLUME_CAPACITY=${JENKINS_PVC_SIZE} --param DISABLE_ADMINISTRATIVE_MONITORS=true | oc create -f -
...
echo "Create secrets for Jenkins to access Nexus"
oc create -f - << EOF
apiVersion: v1
kind: Secret
metadata:
...
echo "Link Nexus' secret to puller"
for project in "${PROJECTS[@]}"
do
